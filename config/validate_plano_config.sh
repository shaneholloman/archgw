...
for file in $(find . -name config.yaml -o -name arch_config_full_reference.yaml); do
  ...
  if ! docker run --rm -v "$(pwd)/${file}:/app/arch_config.yaml:ro" -v "$(pwd)/${file}_rendered:/app/arch_config_rendered.yaml:rw" --entrypoint /bin/sh katanemo/plano:0.4.2 -c "python -m planoai.config_generator" 2>&1 > /dev/null ; then
